...
function p.send(name, value)
  local shader = love.graphics.getShader()
  if shader and type(value) ~= nil and shader:getExternVariable(name) then
...
function p.stitch(source, file)
  return source .. "\n" .. love.filesystem.read(file)
end

function p.stitchDir(source, dir)
  for _, file in pairs(love.filesystem.getDirectoryItems(dir)) do
    source = p.stitch(source, dir .. "/" .. file)
  end
  return source
end
